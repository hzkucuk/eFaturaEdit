...
- **🤖 Claude Code motoru (v2.35.0, opt-in)** — Klasör Ajanı'nın üçüncü modu: sağlayıcı yerine gerçek `claude` ikilisi sürülür (Claude.ai aboneliği veya Anthropic anahtarıyla). Her araç çağrısı bir **PreToolUse hook** üzerinden onaydan geçer; eşzamanlı onaylar **kuyrukta** gösterilir. Motor kuruluma gömülmez, ilk kullanımda indirilir (**açılmadan önce sha512** ile doğrulanır). Onay kapısı **fail-closed**; sandbox (mac/Linux) klasör dışına yazmayı engeller ama okumayı engellemez — bilgilendirme bunu dürüstçe söyler.
- **📂 Editör entegrasyonu + oturum sürekliliği (v2.36–2.37)** — Claude Code motoru ürettiği `.xslt`/`.xml`'i **uygulamanın editör sekmesinde açar** (şablon + verisini birlikte, aynı sekmeye — önizleme hemen derlenir). Konuşma `--resume` ile **sürer**, önceki oturumlar **listeden geri yüklenir**; model ve düşünme eforu Ayarlar'dan seçilir. **Yazma kilidi artık onay kapısında zorlanır:** `Write/Edit` klasör dışına **yazamaz** (ölçüldü: OS sandbox'ı yalnız `bash`'i kısıtlıyor, yerleşik dosya araçlarını değil).
- **🖥️ Terminal görünümü (v2.37, opt-in)** — Claude Code içinde `[Panel | Terminal]`: gerçek interaktif `claude` arayüzü uygulamanın içinde (xterm + PTY). ⚠️ **Açıkça etiketli ham görünüm** — interaktif modda klasör kilidi **garanti değildir** (ölçüldü); tek koruma claude'un kendi onay promptlarıdır. Güvenli deneyim **Panel**'dir.
- **📁 Dosya Gezgini (v2.38.0)** — sol panel sekmeli: **Snippet'ler | Gezgin**. Ajanın çalışma klasörünü ağaçta gösterir; ajanın dokunduğu dosyalar **Y** (oluşturdu) / **M** (değiştirdi) / **S** (silindi) rozetleriyle işaretlenir (git değil — fatura klasörleri genelde git deposu olmadığı için "ajan neye dokundu" izlenir). Tıklayınca editörde açılır ve **modele bağlam olarak bildirilir**.
